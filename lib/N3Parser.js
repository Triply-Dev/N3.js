...
function N3Parser(options) {
  ...
  this._lexer =
    options.lexer || new N3Lexer({ lineMode: isLineMode, n3: isN3 });
...
N3Parser.prototype = {
  ...
  _readEntity: function (token, quantifier) {
    ...
    case 'typeIRI':
      value =
          this._base === null || absoluteIRI.test(token.value) ?
            token.value
            : this._resolveIRI(token);
      break;
  ...
  _readListItem: function (token) {
    ...
    case '[':
      ...
      this._saveContext(
          'blank',
          this._graph,
          (list = '_:b' + blankNodeCount++),
          RDF_FIRST,
          (this._subject = item = '_:b' + blankNodeCount++)
        );
    ...
    case ')':
      ...
      if (this._predicate === null) {
        ...
        if (this._subject === RDF_NIL) return next;
      }
      else {
          // The list was in the parent context's object
        next = this._getContextEndReader();
          // No list tail if this was an empty list
        if (this._object === RDF_NIL) return next;
      }
    ...
    if (list === null) this._subject = list = '_:b' + blankNodeCount++;
    ...
    if (item !== null) {
      ...
      if (itemComplete) this._triple(list, RDF_FIRST, item, this._graph);
      else
  ...
  _completeLiteral: function (token, listItem) {
    ...
    if (suffix) return this._getContextEndReader();
    else {
  ...
  _readPunctuation: function (token) {
    ...
    default:
      ...
      return this._error(
          'Expected punctuation to follow "' + this._object + '"',
          token
        );
    }
    ...
    if (subject !== null) {
      var predicate = this._predicate,
          object = this._object;
      if (!inversePredicate) this._triple(subject, predicate, object, graph);
      else this._triple(object, predicate, subject, graph);
    }
    return next;
  },
  ...
  _readBackwardPath: function (token) {
    var subject = '_:b' + blankNodeCount++,
        predicate,
        object;
    // The next token is the predicate
    if ((predicate = this._readEntity(token)) === undefined) return;
  ...
  parse: function (input, tripleCallback, prefixCallback) {
    ...
    this._lexer.tokenize(input, function (error, token) {
      if (error !== null) self._callback(error), (self._callback = noop);

...
function N3Lexer(options) {
  if (!(this instanceof N3Lexer)) return new N3Lexer(options);
  options = options || {};
  ...
  if (options.lineMode) {
    ...
    this.tokenize = function (input, callback) {
      this._tokenize(input, function (error, token) {
        if (
          !error &&
          /^(?:IRI|prefixed|literal|langcode|type|\.|eof)$/.test(token.type)
        )
          callback && callback(error, token);
        else
          callback &&
            callback(error || self._syntaxError(token.type, (callback = null)));
      });
    };
  }
...
N3Lexer.prototype = {
    ...
    while (true) {
      ...
      case '-':
        ...
        if (
          ...
          value =
              ...
              (match[1] ?
                'double'
                : /^[+\-]?\d+$/.test(match[0]) ? 'integer' : 'decimal');
        }
        break;
      ...
      case 't':
        ...
        if ((match = this._boolean.exec(input)))
          (type = 'literal'), (value =
              '"' + match[0] + '"^^http://www.w3.org/2001/XMLSchema#boolean');
        else inconclusive = true;
        break;
      ...
      if (inconclusive) {
        ...
        if (
          ...
          // Therefore, try inserting a space if we're at the end of the input.
          (type = 'prefixed'), (prefix =
            match[1] || ''), (value = this._unescape(match[2]));
      }
  ...
  _unescape: function (item) {
    try {
      return item.replace(escapeSequence, function (
        sequence,
        unicode4,
        unicode8,
        escapedChar

...
function N3Writer(outputStream, options) {
  if (!(this instanceof N3Writer)) return new N3Writer(outputStream, options);
...
N3Writer.prototype = {
  ...
  tripleToString: function (subject, predicate, object, graph) {
    return (
      this._encodeIriOrBlankNode(subject) +
      ' ' +
  ...
  _encodeObject: function (object) {
    ...
    if (object[0] !== '"') return this._encodeIriOrBlankNode(object);
    // Represent a literal
  ...
  addTriples: function (triples) {
    for (var i = 0; i < triples.length; i++) this.addTriple(triples[i]);
  },

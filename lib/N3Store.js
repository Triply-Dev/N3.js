...
function N3Store(triples, options) {
  if (!(this instanceof N3Store)) return new N3Store(triples, options);
  ...
  if (!options && triples && !triples[0]) (options = triples), (triples = null);
  options = options || {};
...
N3Store.prototype = {
        ...
        for (var value1 in index1) {
          ...
          if ((index2 = index1[value1])) {
            ...
            for (var l = values.length - 1; l >= 0; l--) {
              ...
              else if (callback(result)) return true;
            }
          }
        }
      }
  ...
  addTriple: function (subject, predicate, object, graph) {
    ...
    if (!predicate)
      (graph = subject.graph), (object = subject.object), (predicate =
        subject.predicate), (subject = subject.subject);
  ...
  addTriples: function (triples) {
    for (var i = triples.length - 1; i >= 0; i--) this.addTriple(triples[i]);
  },
  ...
  removeTriples: function (triples) {
    for (var i = triples.length - 1; i >= 0; i--) this.removeTriple(triples[i]);
  },
